...
@RestController
@RequestMapping(value = Route.BASE+Route.CLIENTE, produces = MediaType.APPLICATION_JSON_VALUE)
@CrossOrigin(origins = "*")
public class ClienteController {
	...
	public ResponseEntity<ResponseMainDto> modificarTodo(@RequestBody ClienteModificarDto cliente) {
		Long idResponse = clienteSvc.modificarTodo(cliente);
		if(idResponse != null) {
			return new ResponseEntity<ResponseMainDto>(new ResponseMainDto
					(Constantes.ESTADO_MODIFICACION_EXITOSA, idResponse), HttpStatus.OK);
		} else {
			return new ResponseEntity<ResponseMainDto>(new ResponseMainDto
					(Constantes.ESTADO_MODIFICACION_FALLIDA, Constantes.ESTADO_NOEXISTE), HttpStatus.NOT_FOUND);
		}	
	}
	
	@DeleteMapping(value = Route.ELIMINAR)
	public ResponseEntity<ResponseMainDto> eliminar(@PathVariable String id) {
		Long idResponse = clienteSvc.eliminar(Long.parseLong(id));
		if(idResponse != null) {
			return new ResponseEntity<ResponseMainDto>(new ResponseMainDto
	...
	public ResponseEntity<ResponseMainDto> consultarTodos() {
		return new ResponseEntity<ResponseMainDto>(new ResponseMainDto
				(Constantes.ESTADO_CONSULTA_EXITOSA, clienteSvc.consultarTodos()), HttpStatus.OK);
	}
	
	@PostMapping(value = Route.REGISTRAR)
	public ResponseEntity<ResponseMainDto> registrar(@RequestBody ClienteRegistrarDto cliente) {
		Map<String, Object> map = clienteSvc.registrar(cliente);
		if(map.get("existente") != null) {
			return new ResponseEntity<ResponseMainDto>(new ResponseMainDto
					(Constantes.ESTADO_SIEXISTE, Constantes.DUPLICIDAD_REGISTRO), HttpStatus.BAD_REQUEST);
		}
		if(map.get("response") != null) {
			...
			return new ResponseEntity<ResponseMainDto>(new ResponseMainDto(Constantes.ESTADO_REGISTRO_FALLIDO, 
					ArmarMapResponse.armarRegistroFallidoCliente(map)), HttpStatus.BAD_REQUEST);
		}
	}
