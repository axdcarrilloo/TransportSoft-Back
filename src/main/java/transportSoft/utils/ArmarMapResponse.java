...
public class ArmarMapResponse {
	...
	public static String armarRegistroFallidoEntregaMaritima(Map<String, Object> map) {
		log.info("ArmarMapResponse.class : armarRegistroFallidoEntregaTerrestre() -> Armando String en registro fallido");
			...
			if(map.get("flota") != null) {
				if(response.length() == maxCaracteres) {
					response = response + map.get("flota");
				} else {
					response = response +", "+ map.get("flota");
				}
			}
			if(map.get("puertoEntrega") != null) {
				if(response.length() == maxCaracteres) {
					response = response + map.get("puertoEntrega");
				} else {
					response = response +", "+ map.get("puertoEntrega");
				}
			}
